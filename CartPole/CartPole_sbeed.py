...
def parse_args():
    ...
    parser.add_argument("--baseline_path", type=str, default="comparison_data.npz", 
...
def visualize_results(sbeed_steps, sbeed_rets, args):
    ...
    try:
        np_steps = np.array(sbeed_steps).reshape(5, -1)
        np_rets = np.array(sbeed_rets).reshape(5, -1)
    except ValueError:
    ...
    plt.fill_between(steps_x, avg_rets - std_rets, avg_rets + std_rets, color='blue', alpha=0.05)
    ...
    save_name = f"sbeed_{args.env}.png"
