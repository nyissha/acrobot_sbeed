...
class Config:
    ...
    ckpt_path: str = "weights/Acrobot-v1_medium.pt" 
    save_name: str = "acrobot/D_acrobot_medium_mixed.npz"
...
if __name__ == "__main__":
    os.makedirs("acrobot", exist_ok=True)
    
    cfg = Config(
        env_id="Acrobot-v1",
        ckpt_path="weights/Acrobot-v1_medium.pt", 
        save_name="acrobot/D_acrobot_medium_mixed.npz",
    ...
    if not os.path.exists(cfg.ckpt_path):
        ...
        print(f"Avg Return: {np.mean(dataset['episode_returns']):.2f} (Expected low due to high random ratio)")
        print("-" * 40)